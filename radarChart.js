...
function RadarChart(data, options) {
  var cfg = {
    ...
    hover: true,
    standaloneMode: true, //Create svg element or not
    axisLabels: true,
    tickLabels: true,
    fields: false,
    scalesAndAxes: false,
  ...
  function chartInner(selection){
    ...
    if (cfg.fields != false){
      data = chartInner.subsetAndSortData(data, cfg.fields);
    } else {
    ...
    if (cfg.scalesAndAxes === false){
      autos = generateScalesAndAxes(data);
    } else {
      ...
      blobCircleWrapper.selectAll(".radarInvisibleCircle")
        ...
        .on("mouseover", function(d,i) {
          ...
          tooltip
            ...
            .attr("x", newX)
            .attr("y", newY - 15)
            .text(function(x){return cfg.fields[i];});
 ...
 *
 */
  chartInner.subsetAndSortData = function(data, fields){
    data = data.map(function(row){
      var newRow = {};
...
function generateScalesAndAxes(data){
